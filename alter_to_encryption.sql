-- =========================================================================
-- Supabase SQL Migration Script: Enable Encryption Compatibility
...
-- Add required security/portal columns to EMPLOYEE table
...
-- Add security performance indexes
...
-- 9. Alter QR_CODE column to TEXT to support signed QR code payloads
ALTER TABLE qr_code ALTER COLUMN qr_value TYPE TEXT;

-- 10. Widen tds_details.quarter from VARCHAR(5) to TEXT
--     (the default schema uses VARCHAR(5) which only fits 'Q1'-'Q4')
--     Run this so the column can store longer labels if needed in the future.
ALTER TABLE tds_details ALTER COLUMN quarter TYPE TEXT;

-- 11. Create MANAGERS table required for Manager Authentication and Portal
